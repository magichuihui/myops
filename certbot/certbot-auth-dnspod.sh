...
USER_AGENT="AnDNS/1.0.0 (admin@baiyjk.com)"
DOMAIN=$(expr match "$CERTBOT_DOMAIN" '.*\.\(.*\..*\)')
[ -z "$DOMAIN" ] && DOMAIN="$CERTBOT_DOMAIN"

if [ -z "$API_TOKEN" ]; then
    [ -f /etc/certbot-dnspod/dnspod_token_$DOMAIN ] && API_TOKEN=$(cat /etc/certbot-dnspod/dnspod_token_$DOMAIN)
fi

if [ -z "$API_TOKEN" ]; then
    [ -f /etc/certbot-dnspod/dnspod_token ] && API_TOKEN=$(cat /etc/certbot-dnspod/dnspod_token)
fi
